...
uint MyRigidBody::SAT(MyRigidBody* const a_pOther)
{
	...
	//common variables between different cases
	vector3 v3_thisHalfWidth = this->GetHalfWidth();
	vector3 v3_otherHalfWidth = a_pOther->GetHalfWidth();
	vector3 v3_thisCenter = this->GetCenterGlobal();
	vector3 v3_otherCenter = a_pOther->GetCenterGlobal();
	matrix4 m4_thisModel = this->GetModelMatrix();
	matrix4 m4_otherModel = a_pOther->GetModelMatrix();

	//local axes for both rigidBodies
	vector3 v3_thisLocalAxes[3];
	v3_thisLocalAxes[0] = vector3(m4_thisModel * vector4(AXIS_X, 0.f));
	v3_thisLocalAxes[1] = vector3(m4_thisModel * vector4(AXIS_Y, 0.f));
	v3_thisLocalAxes[2] = vector3(m4_thisModel * vector4(AXIS_Z, 0.f));

	vector3 v3_otherLocalAxes[3];
	v3_otherLocalAxes[0] = vector3(m4_otherModel * vector4(AXIS_X, 0.f));
	v3_otherLocalAxes[1] = vector3(m4_otherModel * vector4(AXIS_Y, 0.f));
	v3_otherLocalAxes[2] = vector3(m4_otherModel * vector4(AXIS_Z, 0.f));

	matrix3 rotation;
	matrix3 absRotation;

	//rotation and absolute rotation from one local space to another's
	for (uint i = 0; i < 3; ++i)
	{
		for (uint j = 0; j < 3; ++j)
		{
	...
	//translation from one object to another
